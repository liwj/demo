...
public class RandomStrg {
	...
	private final void generaterandom() {
		if (allchars)
			...
			if (upper.size() == 3) {
				...
				for (int i = 0; i < length.intValue(); i++) {
					// you have four groups to choose a random number from, to make the choice a little more random select a number out of
				...
				// single chars are to be included choose a random char from two different ranges build the random char from single chars and two ranges
				for (int i = 0; i < length.intValue(); i++) {
					// select the single chars or a range to get each random char from
					if (((int) (getFloat() * 100)) % 2 == 0) {
	 ...
	 * 
	 */
	public final void setHmap(HashMap<String, ?> map) {
		hmap = map;
	}
